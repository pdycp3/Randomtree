...
Tree::Tree(int MaxDepth, int trainFeatureNumPerNode, int minLeafSample, float minInfoGain, bool isRegression, int treeId)
{
	_MaxDepth = MaxDepth;
	_trainFeatureNumPerNode = trainFeatureNumPerNode;
	_minLeafSample = minLeafSample;
...
Result Tree::predict_tree(float * data, int & nNodeId)
{
	int position = 0;
	Node*head = _cartreeArray[position];
	while (!head->isLeaf())
...
ClasTree::ClasTree(int MaxDepth, int trainFeatureNumPerNode,int minLeafSample,float minInfoGain,bool isRegression,int treeId)
	:Tree(MaxDepth,trainFeatureNumPerNode,minLeafSample,minInfoGain,isRegression,treeId)
{}
ClasTree::~ClasTree()
{}
void ClasTree::train(Sample*sample)
{
	...
	for(int i=0;i<_nodeNum;++i)
	{
		...
		{
			_cartreeArray[i]->createLeaf();
			_cartreeArray[i]->createLeafWithId();
			continue;
		}
		//随机选择样本维度id
