...
public:
	...
	**************************************************************/
	...
	Tree(int MaxDepth, int trainFeatureNumPerNode, int minLeafSample, float minInfoGain, bool isRegression,int treeId);
	virtual ~Tree();
	virtual void train(Sample*Sample)=0;
	Result predict(float*data);
	Result predict_tree(float * data, int & nNodeId);
	inline Node**getTreeArray(){return _cartreeArray;};
	virtual void createNode(int id,int featureIndex,float threshold)=0;
	int getTreeId() { return _treeId; }
protected:
...
public:
	...
	ClasTree(int MaxDepth,int trainFeatureNumPerNode,int minLeafSample,float minInfoGain,bool isRegression,int treeId);
	~ClasTree();
